...
<template>
  <div class="mt-4">
    ...
    <div v-if="activeMode === 'rpe'" class="mt-3">
      <OptionButtonGroup
    ...
    <label v-else-if="activeMode === 'percent'" class="mt-3 block max-w-xs text-xs text-slate-500">
      % du 1RM
      <input
    ...
    <label v-else-if="activeMode === 'kg'" class="mt-3 block max-w-xs text-xs text-slate-500">
      Charge (kg)
      <input
        :value="line.load ?? ''"
        type="number"
    ...
    <p v-else class="mt-2 text-xs text-slate-500">Choisis un type de charge ci-dessus.</p>
  </div>
</template>
